...
impl<'a> CandleChart<'a> {
    ...
    pub fn new(candles: &'a Candles, mouse_event: Option<MouseEvent>) -> Self {
        Self { candles, mouse_event }
    }

    fn render_chart(candles: impl Iterator<Item = &'a Candle>, scale: &PriceScale, area: Rect, buf: &mut Buffer) {
    ...
    fn render_price(crosshair: &Crosshair, scale: &PriceScale, plot_area: Rect, axis_area: Rect, buf: &mut Buffer) {
        let local_row = crosshair.row.saturating_sub(plot_area.y).min(plot_area.height.saturating_sub(1));
...
impl<'a> Widget for CandleChart<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let [header_area, chart_area] = Layout::vertical([Constraint::Length(1), Constraint::Min(1)]).areas(block_area);

        let [plot_area, axis_area] = Layout::horizontal([Constraint::Min(1), Constraint::Length(Self::PRICE_AXIS_WIDTH)]).areas(chart_area);
...
impl Cell {
    pub fn new(row: u16, body: &RangeInclusive<u16>, wick: &RangeInclusive<u16>, is_doji: bool) -> Self {
...
impl<'a> Crosshair<'a> {
    pub fn new(mut candles: impl Iterator<Item = &'a Candle>, area: Rect, mouse: MouseEvent) -> Option<Self> {
...
impl PriceScale {
    pub fn new<'a>(candles: impl Iterator<Item = &'a Candle>, area_height: u16) -> Self {
        let (min, max) = candles.fold((f64::MAX, f64::MIN), |(lo, hi), c| (lo.min(c.low), hi.max(c.high)));
