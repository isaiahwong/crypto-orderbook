...
impl<'a> DomWidget<'a> {
    ...
    fn depth<'b>(price: &str, amount: &str, total: f64, max_cumulative: f64, area_width: u16, is_bid: bool) -> Line<'b> {
        let price_color = if is_bid { Color::Green } else { Color::Red };
        let bg_color = if is_bid { Color::Rgb(0, 50, 0) } else { Color::Rgb(50, 0, 0) };
        ...
        for (i, c) in full_text.chars().enumerate() {
            ...
            let bg = if i < bar_width { bg_color } else { Color::Reset };
    ...
    fn render_side(area: Rect, ob: &Orderbook, is_bid: bool, buf: &mut ratatui::buffer::Buffer) {
        ...
        let side = if is_bid { ob.bids(limit) } else { ob.asks(limit) };
    ...
    fn render_mid(area: Rect, ob: &Orderbook, buf: &mut ratatui::buffer::Buffer) {
        ...
        if let (Some(mid), Some(spread)) = (ob.mid(), ob.spread()) {
            ...
            mid_text = format!("Mid: {:.4} | Spread: {:.2} ({:.3}%)", mid, spread, spread_pct);
        }
...
impl<'a> Widget for DomWidget<'a> {
    fn render(self, area: Rect, buf: &mut ratatui::buffer::Buffer) {
        ...
        let cols = Layout::default()
            .direction(Direction::Vertical)
            .constraints([Constraint::Percentage(45), Constraint::Percentage(5), Constraint::Percentage(45)])
            .split(block_area);
